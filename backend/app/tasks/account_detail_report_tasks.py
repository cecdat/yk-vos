...
def sync_account_detail_reports_daily(self):
    """
    每天定时同步所有VOS节点上所有账户的前一天明细报表
    ...
    try:
        ...
        begin_time_str = yesterday.strftime('%Y%m%d')
        end_time_str = yesterday.strftime('%Y%m%d')
        
        logger.info(f"📊 开始同步账户明细报表，日期: {yesterday}")
        ...
        for instance in instances:
            ...
            try:
                logger.info(f"🔄 同步VOS实例 {instance.name} (ID={instance.id}) 的账户明细报表")
                ...
                if not customers:
                    logger.warning(f"VOS实例 {instance.name} 没有账户数据，跳过")
                    continue
                
                # 获取账户列表
                account_list = [c.account for c in customers if c.account]
                if not account_list:
                    logger.warning(f"VOS实例 {instance.name} 没有有效的账户号码，跳过")
                    continue
                ...
                logger.error(f"同步VOS实例 {instance.name} 的账户明细报表失败: {e}", exc_info=True)
                db.rollback()
                total_errors += 1
                continue
        
        logger.info(f"📊 账户明细报表同步任务完成")
        logger.info(f"   成功同步: {total_synced} 条记录")
        logger.info(f"   失败: {total_errors} 个实例")
