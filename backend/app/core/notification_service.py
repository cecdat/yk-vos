...
def get_notification_service(db):
    ...
    # 获取推送项目和类型配置
    # 默认值为 'all'，但如果数据库中有配置且为空字符串，则应该尊重用户的选择（即不推送）
    push_projects = 'all'
    push_projects_config = db.query(AppConfig).filter(AppConfig.config_key == 'notification_push_projects').first()
    if push_projects_config:
        # 如果配置存在（即使为空字符串），则使用配置值
        push_projects = push_projects_config.config_value if push_projects_config.config_value is not None else ''
    
    push_types = 'all'
    push_types_config = db.query(AppConfig).filter(AppConfig.config_key == 'notification_push_types').first()
    if push_types_config:
        # 如果配置存在（即使为空字符串），则使用配置值
        push_types = push_types_config.config_value if push_types_config.config_value is not None else ''
